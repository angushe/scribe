...
#define CALC_LOSS() do {                    \
  ...
  if (!inputBuffer) {
    ...
    if (inputBuffer == NULL) {
      CALC_LOSS();
      LOG_WARNING("WARNING: nomem Data Loss loss %ld bytes in %s", size,
          filename.c_str());
     return (size);
    }
  }
  ...
  if (size >= INT_MAX) {
    ...
    CALC_LOSS();
    LOG_WARNING("WARNING: Corruption Data Loss %ld bytes in %s", size,
        filename.c_str());
    return (size);
  }
  ...
  if (size > bufferSize) {
    ...
    if (bufferSize > LARGE_BUFFER_SIZE) {
      LOG_WARNING("WARNING: allocating large buffer Corruption? %d", bufferSize);
    }
  }
  if (inputBuffer == NULL) {
    CALC_LOSS();
    LOG_WARNING("WARNING: nomem Corruption? Data Loss %ld bytes in %s", size,
        filename.c_str());
    return (size);
  }
  ...
  if (file.good()) {
    ...
    CALC_LOSS();
    LOG_WARNING("WARNING: Data Loss %ld bytes in %s", size, filename.c_str());
  }
  if (bufferSize > LARGE_BUFFER_SIZE) {
    free(inputBuffer);
    inputBuffer = NULL;
...
unsigned long StdFile::fileSize() {
  ...
  try {
    ...
    LOG_WARNING("Failed to get size for file <%s> error <%s>", filename.c_str(), e.what());
    size = 0;
  }
  return size;
}
...
void StdFile::listImpl(const std::string& path, std::vector<std::string>& _return) {
  try {
    ...
    LOG_WARNING("exception <%s> listing files in <%s>",
             e.what(), path.c_str());
  }
}
...
bool StdFile::createDirectory(std::string path) {
  try {
    ...
    LOG_WARNING("Exception < %s > in StdFile::createDirectory for path %s ",
      e.what(),path.c_str());
    return false;
  }
